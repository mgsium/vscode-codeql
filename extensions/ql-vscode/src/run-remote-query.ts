...
export async function runRemoteQuery(cliServer: cli.CodeQLCliServer, credentials: Credentials, uri?: Uri) {
  ...
  await runRemoteQueriesApiRequest(credentials, ref, language, repositories, query, queryPackBase64);
}

async function runRemoteQueriesApiRequest(credentials: Credentials, ref: string, language: string, repositories: string[], query: string, queryPackBase64: string): Promise<void> {
  ...
  try {
    ...
    await attemptRerun(error, credentials, ref, language, repositories, query, queryPackBase64);
  }
}

/** Attempts to rerun the query on only the valid repositories */
export async function attemptRerun(error: any, credentials: Credentials, ref: string, language: string, repositories: string[], query: string, queryPackBase64: string) {
  if (typeof error.message === 'string' && error.message.includes('Some repositories were invalid')) {
    ...
    if (rerunQuery) {
      ...
      await runRemoteQueriesApiRequest(credentials, ref, language, validRepositories, query, queryPackBase64);
    }
